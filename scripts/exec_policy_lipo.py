...
+ Quaternion filtering and SE(3) step limiting applied before execution
"""
import sys
import os
import pathlib
...
class ActionLiPo:
    def __init__(self, solver="CLARABEL", 
        ...
        self.JM = JM
        self.epsilon = cp.Variable((self.N+JM, self.D))  # previous +3 to consider vel/acc/jerk
        ...
        q_total = self.epsilon + self.ref  # (N+JM, D)
    ...
    def solve(self, actions: np.ndarray, past_actions: np.ndarray, len_past_actions: int):
        ...
        Parameters:
        - actions: (N, D)
        - past_actions: (N, D) previously optimized chunk (used for blending)
        - len_past_actions: B, how many steps to blend
        Returns:
        - solved: optimized actions (N, D)
        - ref: reference sequence used (N, D)
        """
        blend_len = len_past_actions
        JM = self.JM
        self.ref.value[JM:] = actions.copy()
        
        if blend_len > 0 and past_actions is not None:
            # update last actions
            self.ref.value[:JM+self.TD] = past_actions[-blend_len-JM:-blend_len + self.TD].copy()
            ratio_space = np.linspace(0, 1, blend_len-self.TD)  # (B-TD,)
            ...
            # no blending with past: hold the first action for the prepended margin
...
def main():
    ...
    # LiPo Configuration
    lipo_chunk_size = 18              # Larger than steps_per_inference to allow overlap
    lipo_blending_horizon = 8         # How many steps to blend between chunks
    lipo_time_delay = 2               # Account for inference latency (in timesteps)
    ...
    lipo = ActionLiPo(
        ...
        action_dim=7,  # action: [x y z rx ry rz g]
        len_time_delay=lipo_time_delay,
        dt=dt,
        epsilon_blending=0.025,  # allow more deviation in blending zone
        epsilon_path=0.003       # tight constraint for main trajectory
    )
    ...
    with SharedMemoryManager() as shm_manager:
        with KeystrokeCounter() as key_counter, \
            ...
            # Optional: match dataset preview load
            ...
            # Bootstrap one prediction (optional)
            with torch.no_grad():
                policy.reset()
                ...
                action0 = result['action_pred'][0].detach().to('cpu').numpy()
                action0 = get_real_umi_action(action0, obs, action_pose_repr)
                assert action0.shape[-1] == 7
                del result
                ...
                try:
                    ...
                    while True:
                        ...
                        with torch.no_grad():
                            ...
                            action_chunk = get_real_umi_action(raw_action, obs, action_pose_repr)  # [H,7]
                            del result
                            
                            # Extend/trim to LiPo chunk size
                            if len(action_chunk) < lipo_chunk_size:
                                last_action = action_chunk[-1:]
                                pad = np.repeat(last_action, lipo_chunk_size - len(action_chunk), axis=0)
                                extended_action_chunk = np.concatenate([action_chunk, pad], axis=0)
                            else:
                                extended_action_chunk = action_chunk[:lipo_chunk_size]
                            
                            # Apply LiPo smoothing
                            if prev_optimized_chunk is not None:
                                smoothed_actions, _ = lipo.solve(
                            ...
                            if smoothed_actions is not None:
                                ...
                                candidate_actions = smoothed_actions[:steps_per_inference]
                                print(f"Applied LiPo smoothing (chunk {action_chunk_counter})")
                            else:
                                candidate_actions = action_chunk[:steps_per_inference]
                                print("LiPo failed, using original actions")
                            
                            action_chunk_counter += 1
                            
                            # Timestamp targets relative to latest observation
                            action_timestamps = (np.arange(len(candidate_actions), dtype=np.float64)) * dt + obs_timestamps[-1]

                            # ===== Quaternion + SE(3) limiting =====
                            # use last gripper command for all substeps in this cycle
                            g_now = float(candidate_actions[-1, 6]) if candidate_actions.ndim == 2 else float(candidate_actions[6])
                            ...
                            if np.sum(is_new) == 0:
                                # Over budget -> send one safe step aligned to next tick
                                idx = -1
                                p_cmd = candidate_actions[idx][:3]
                                q_cmd = R.from_rotvec(candidate_actions[idx][3:6]).as_quat()

                                # limit around last commanded pose
                                p_safe, q_safe = _limit_se3_step(p_last, q_last, p_cmd, q_cmd, v_max, w_max, dt)
                                p_last, q_last = p_safe, q_safe

                                a_exec = candidate_actions[idx].copy()
                                a_exec[:3]  = p_safe
                                ...
                                safe_timestamps = [eval_t_start + next_step_idx * dt]
                                print('Over budget', safe_timestamps[0] - curr_time)

                            else:
                                # filter valid future targets
                                candidate_actions = candidate_actions[is_new]
                                action_timestamps = action_timestamps[is_new]

                                # per-step SE(3) rate limiting
                                for a, tstamp in zip(candidate_actions, action_timestamps):
                                    p_cmd = a[:3]
                                    q_cmd = R.from_rotvec(a[3:6]).as_quat()

                                    # rate limit (updates p_last/q_last)
                                    p_safe, q_safe = _limit_se3_step(p_last, q_last, p_cmd, q_cmd, v_max, w_max, dt)
                                    p_last, q_last = p_safe, q_safe
                                    ...
                                    safe_timestamps.append(tstamp)

                            # Log
                            for a, t in zip(this_target_poses, safe_timestamps):
                                a = a.tolist()
                                action_log.append({
                                    'timestamp': t,
                                    'ee_pos_0': a[0],
                            ...
                            env.exec_actions(
                                ...
                                timestamps=np.array(safe_timestamps, dtype=np.float64),
                                compensate_latency=True
                            )
                            print(f"Submitted {len(this_target_poses)} SE(3)-limited steps.")
