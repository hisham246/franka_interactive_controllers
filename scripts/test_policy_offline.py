...
import os
import glob
from policy_utils.pose_repr_util import convert_pose_mat_rep
from policy_utils.pose_util import (
    pose_to_mat, mat_to_pose10d)

from policy_utils.real_inference_util import get_real_umi_action
from policy_utils.pytorch_util import dict_apply
import torch
from matplotlib.animation import FuncAnimation

def save_episode_predictions(results, out_csv_path, include_quat=False):
    """
    Save model predictions (first action step per timestep) to CSV.

    CSV columns (always): time, x, y, z, rx, ry, rz
    If include_quat=True, also saves: qx, qy, qz, qw

    - results['predicted_actions'] is a list (len=T) of lists (len=H)
      where each inner element is a 7D executable action [x,y,z,rx,ry,rz,(optional gripper)]
    """
    ...
    for t in range(T):
        ...
        if len(actions) == 0:
            # no prediction — fill NaNs
            if include_quat:
                rows.append([results['timestamps'][t]] + [float('nan')]*10)
            else:
                rows.append([results['timestamps'][t]] + [float('nan')]*6)
            continue

        first = actions[0]  # first step of action horizon
        x, y, z = first[0], first[1], first[2]
        rx, ry, rz = first[3], first[4], first[5]

        if include_quat:
            quat = rotvec_to_quat(np.array([rx, ry, rz]))  # [qx, qy, qz, qw]
            rows.append([results['timestamps'][t], x, y, z, rx, ry, rz,
    ...
    if include_quat:
        cols = ['time','x','y','z','rx','ry','rz','qx','qy','qz','qw']
    else:
        cols = ['time','x','y','z','rx','ry','rz']
...
def run_policy_simulation(dataset_directory, obs_pose_repr='relative', action_pose_repr='relative', 
    ...
    try:
        ...
        # Apply the same config overrides as your inference script
    ...
    episode_dir = episode_dirs[20]  # Just take the first episode
    ...
    try:
        ...
        # (Optional) also save quaternions for sanity checks or ROS publishers expecting quats
...
# Add the process_episode method to the simulator class
def process_episode_method(self, episode_dir):
    ...
    # Simulate policy execution at each timestep
    for t in range(len(positions)):
        # Process observations (convert to policy input format)
        ...
        # Convert each predicted action step
        executable_actions = []
        for action_step in predicted_action_10d:
            real_action = get_real_umi_action(
                action_step, env_obs, self.action_pose_repr
...
if __name__ == "__main__":
    ...
    dataset_directory = "/home/hisham246/uwaterloo/umi/surface_wiping_tp/dataset/"
    
    # Path to your trained policy checkpoint (REQUIRED)
    policy_path = "/home/hisham246/uwaterloo/diffusion_policy_models/surface_wiping_transformer_position_control.ckpt"
