...
class FrankaROSInterface:
    ...
    def get_joint_velocities(self):
        while self.joint_velocities is None and not rospy.is_shutdown():
            rospy.sleep(0.01)
    ...
    def start_cartesian_impedance(self, Kx: np.ndarray, Kxd: np.ndarray):
        self.update_current_policy(Kx, Kxd)

    def update_impedance_gains(self, Kx: np.ndarray, Kxd: np.ndarray):
        self.update_current_policy(Kx, Kxd)

    def update_current_policy(self, Kx: np.ndarray, Kxd: np.ndarray):
        config = {
    ...
    def terminate_current_policy(self):
        # You can optionally reset impedance or stop publishing here
        rospy.loginfo("[FrankaRosInterface] No terminate command needed for ROS controller.")
