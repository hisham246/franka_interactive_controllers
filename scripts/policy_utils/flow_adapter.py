...
class DiffusionAsFlow(nn.Module):
    """
    ...
    def _eps_x0_from_model(self, z_t, t_id, global_cond):
        ...
        model_out = self.model(z_t, t_id, local_cond=None, global_cond=global_cond)
        ...
        if self.pred_type == 'epsilon':         # model predicts ε̂
            eps_hat = model_out
            # x0_hat = (z_t - sqrt_one_minus_ab * eps_hat) / (sqrt_ab + 1e-8)
            x0_hat = (z_t - sqrt_one_minus_ab * eps_hat) / (sqrt_ab)
        else:                                   # 'sample' — model predicts x̂0
            x0_hat = model_out
            # eps_hat = (z_t - sqrt_ab * x0_hat) / (sqrt_one_minus_ab + 1e-8)
