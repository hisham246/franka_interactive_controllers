...
from snds.policy_interface import PlanningPolicyInterface
from torch.utils.data import DataLoader, TensorDataset

from tqdm.auto import tqdm

from snds.networks import NN, LNET
from snds.euclidean_flows import init_sdsef_model
from snds.deep_dynamics import joint_lpf_ds_model

from snds.utils import mse
from snds.log_config import logger
...
class NL_DS(PlanningPolicyInterface):
    ...
    def __init__(self, network: str = 'nn', data_dim: int = 3, gpu: bool = True, eps: float = 0.01,
    ...
    def fit(self, trajectory: np.ndarray, velocity: np.ndarray, n_epochs: int = 200,
        ...
        for epoch in (par := tqdm(range(n_epochs))):
            ...
            for trajs_t, vels_t in self.__dataset:
                ...
                if loss > loss_clip:
                    logger.warning('Loss value is too large, reinitializing')
