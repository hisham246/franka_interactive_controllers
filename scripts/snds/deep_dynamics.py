...
class NormalNN(nn.Module):
    def __init__(self, layer_sizes: List[int] = [3, 128, 128, 128, 3],
...
class Calibrate(nn.Module):
    def __init__(self, f, n=3, device=torch.device("cpu")):
        super().__init__()
        self.f = f
...
def joint_lpf_ds_model(device, lsd=3, fhat_layers=[3, 256, 256, 256, 3], lpf_layers=[3, 64, 64, 1], eps: float = 0.01,
